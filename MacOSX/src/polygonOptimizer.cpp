...
int polygonL1Resolution = 0;
int polygonL2Resolution = 0;
...
void optimizePolygon(PolygonRef poly)
{
    ...
    {
        Point p1 = poly[i];
        if (shorterThen(p0 - p1, MICRON2INT(polygonL1Resolution)))
        {
            poly.remove(i);
            i --;
        }else if (shorterThen(p0 - p1, MICRON2INT(polygonL2Resolution)))
        {
            Point p2;
            if (i < poly.size() - 1)
                p2 = poly[i+1];
